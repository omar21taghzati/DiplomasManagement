...
@section('content')
    ...
    <div class="container-fluid py-16">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-3 mb-3">
                <div class="card-summary text-center">
                    <h5>Reserved Bacs</h5>
                    <div class="count  text-danger">{{ $reserved }}</div>
                    <div class="change text-danger">
                        {{ $total > 0 ? number_format(($reserved * 100) / $total, 2) : '0.00' }}%</div>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="card-summary text-center">
                    <h5>Delivered Bacs</h5>
                    <div class="count  text-success">{{ $delivered }}</div>
            ...
            <div class="col-md-3 mb-3">
                <div class="card-summary text-center">
                    <h5>Undelivered Bacs</h5>
                    <div class="count  text-warning">{{ $undelivered }}</div>
        ...
        <form method="POST" class="row g-2 align-items-center filter-form mb-4" id='inputsForm'>
            ...
            <div class="col-md-3">
                <select name="status" class="form-select" id="select-status">
                    <option value="">Bac Status</option>
                    ...
                    @forelse ($statistics as $statistic)
                        <tr>
                            ...
                            <td>
                                ...
                                @php
                                    $status = $statistic->certificat->status;
                                @endphp
    ...
    @push('scripts')
        <script>
            ...
            let filterActive = false; //  Track if filtering is active
            function loadPage(url, formData) {
                axios.post(url, formData)
                    ...
                    `;
                        ...
                        filterActive = true; //  Activate AJAX pagination
                    })
                    .catch(error => {
            ...
            form.addEventListener('submit', function(e) {
                ...
                loadPage('/statistic/bac', formData);
            });
...
{{-- // function loadStatistic(cef) {
              //     const tbody = document.getElementById("fill-body");
              //     tbody.innerHTML = '';
              //     const paginationContainer = document.getElementById("pagination-statistic");
              //     paginationContainer.innerHTML = '';
  
              //     // If no cef, pass empty string (to hit the optional route)
              //     const url = cef ? `/statistic/bac/staigiare/${cef}` : `/statistic/bac/staigiare`;
